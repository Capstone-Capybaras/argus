...
import { threatLandscapeTable, jobsTable, entitiesTable, assetsTable, threatLandscapeGeneratedTable, threatFilesTable } from '../../database/schema';
...
export class ThreatLandscapeService {
  ...
  async getThreatLandscapeByEntityIdAndFile(entity_id: number, file_key: string) {
  ...
  async getLatestThreatLandscape(entity_id: number) {
    ...
    try{
      const threats = await this.getThreatLandscapeByEntityIdAndFile(entity_id, mostRecentFile[0].file_key);
      if (!threats || threats.length===0){
        throw new Error(`No Threatlandscape found for entity: ${entity_id} and file: ${mostRecentFile[0].file_key}`);
      }
      return threats
    } catch(error){
  ...
  async getUploadHistory(entityId:number){
    const uploads = await this.db.select().from(threatFilesTable).where(eq(threatFilesTable.entity_id, entityId));
    return uploads
  }

  async generateThreatLandscape(data: GenerateThreatDto){
    const entity = await this.db.select().from(entitiesTable).where(eq(entitiesTable.id, data.entity_id)).limit(1);
    ...
    const assets = await this.db.select().from(assetsTable).where(eq(assetsTable.entity_id, data.entity_id));
    ...
    const match = data.file_key.match(/_(\d{4}-\d{2}-\d{2})_(\d{2}-\d{2}-\d{2})/);
    if (!match) {
    ...
    const [threatFileEntry] = await this.db.insert(threatFilesTable).values(threatFilesDto).returning();
    if (!threatFileEntry){throw new Error("Error adding threat file to db.")}
