...
export class EmailService {
    ...
    async getEmailsByProject(projectId: number) {
        ...
        const emails = await this.database.select().from(schemas.emailsTable).where(eq(schemas.emailsTable.project_id, projectId));
        const headerFooter = await this.database.select({
          email_header: schemas.projectsTable.email_header,
          email_footer: schemas.projectsTable.email_footer
        }).from(schemas.projectsTable).where(eq(schemas.projectsTable.id, projectId));
    ...
    async getEmailsById(emailId: number) {
      //return this.database.select().from(schemas.emailsTable);
      const email = await this.database.select().from(schemas.emailsTable).where(eq(schemas.emailsTable.id, emailId)).limit(1);
      return email[0] || null;
    }
    ...
    async addEmail(data: CreateMailDto) {
        const result = this.database
          ...
          .values({
            ...
            schedule_date_time: data.scheduleDateTime ? new Date(data.scheduleDateTime) : null,
    ...
    async updateEmail(emailId: number, data : UpdateMailDBDto){
      const result = await this.database.update(schemas.emailsTable).set(data)
    ...
    async updateStatus(emailId: number, status: string){
      const result = await this.database.update(schemas.emailsTable).set({status: status})
        .where(eq(schemas.emailsTable.id, emailId))
        .returning();
      return result || null;
    }
    ...
    async getEmailHeaderFooter(projId: number){
      const result = await this.database.select({
        email_header: schemas.projectsTable.email_header,
        email_footer: schemas.projectsTable.email_footer
      }).from(schemas.projectsTable).where(eq(schemas.projectsTable.id, projId));
      return result.length ? result[0] : null;
    }

    async deleteEmail(emailId: number){
      const result = await this.database.delete(schemas.emailsTable).where(eq(schemas.emailsTable.id, emailId)).returning();
      return result || null;
    }

    async deleteEmailByProj(projId: number){
      const result = await this.database.delete(schemas.emailsTable).where(eq(schemas.emailsTable.project_id, projId)).returning();
      return result || null;
    }
    ...
    async getAttachment(key: string){
      ...
      try{
        ...
        let attachment: AttachmentDto = {filename: filename, content: "", encoding: "base64", contentDisposition:"attachment"}
    ...
    async sendMail(emailId: number) {
        try {
            let email : RawEmail = {from:"<test> simx1@labs.ensigninfosecurity.com", to:[], subject:"", html:"", attachments:[]}
            let emailContent = await this.getEmailsById(emailId);
            ...
            email["html"] = (emailHeaderFooter?.email_header ?? "") + "<br>" + (emailContent.html ?? "") + "<br>" + (emailHeaderFooter?.email_footer ?? "")
