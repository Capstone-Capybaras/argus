...
export class BullQueueService {
  ...
  async updateEmailSchedule(updateMailDto: UpdateMailClient){
    try{
      let data : UpdateMailDBDto = {to: updateMailDto.to, subject: updateMailDto.subject, html: updateMailDto.html, attachments: updateMailDto.attachments};
      let update = await this.emailService.updateEmail(updateMailDto.emailId, data);
      ...
      if (update.schedule_date_time != updateMailDto.scheduleDateTime){
          ...
          if (update.job_id == null && updateMailDto.scheduleDateTime != null){
              ...
              jobId = job.jobId
          } else if (update.job_id != null && updateMailDto.scheduleDateTime != null) {
              const delay = new Date(updateMailDto.scheduleDateTime);
              const newJob = await this.updateJob(update.job_id, updateMailDto.emailId, delay);
              jobId = newJob.jobId
          }
          const scheduleData : UpdateMailDBDto = {
              job_id: Number(jobId),
              schedule_date_time: updateMailDto.scheduleDateTime != null ? new Date(updateMailDto.scheduleDateTime) : null,
              status: "scheduled",
          }
          console.log("scheduleData: ", scheduleData);
          const newEmail = await this.emailService.updateEmail(updateMailDto.emailId, scheduleData);
          update = newEmail;
      }
