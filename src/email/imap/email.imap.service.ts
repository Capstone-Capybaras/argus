...
import { Box, MailBoxes } from 'imap';
...
export class ImapService implements OnModuleDestroy {
  ...
  async openInbox(threadTopic: string, date: string, take: number = 10, skip: number = 0): Promise<{emails: MailParser.ParsedMail[]}> {
    return new Promise((resolve, reject)=>{
      ...
      this.imap.once('ready', ()=>{
        this.imap.openBox('INBOX', true, async (err, box) => {
          if (err) {console.log(String(err)); reject(err)};
  ...
  findAttachmentParts(struct: any, attachments: any[]) {
    ...
    for (var i = 0, len = struct.length, r; i < len; ++i) {
      if (Array.isArray(struct[i])) {
        ...
        if (struct[i].disposition && ['INLINE', 'ATTACHMENT'].indexOf(this.toUpper(struct[i].disposition.type)) > -1) {
  ...
  toUpper(thing: string) { return thing && thing.toUpperCase ? thing.toUpperCase() : thing;}

  async fetchMail(threadTopic: string, date: string, take: number = 10, skip: number = 0): Promise<any>{
    return new Promise((resolve, reject) => {
      ...
      const searchCriteria = [['HEADER', 'THREAD-TOPIC', threadTopic], ['ON', date]];
      ...
      this.imap.search(searchCriteria, async (err, results)=>{
        ...
        if (results.length === 0) {console.log("no mail found"); return this.imap.end();}
        ...
        let emailArray: MailParser.ParsedMail[] = [];
        const emailPromises: Promise<MailParser.ParsedMail>[] = [];

        emails.on("message", (message, seqno)=>{
          emailPromises.push(
            new Promise((resolveMail) => {
              ...
              message.once("attributes", (attrs: Imap.ImapMessageAttributes)=>{
                let parts = this.findAttachmentParts(attrs.struct, []);
                for (var i = 0, len=parts.length ; i < len; ++i) {
                  let attachment = parts[i];
                  console.log('Fetching attachment %s', attachment.params.name);
                  var f = this.imap.fetch(attrs.uid , { //do not use imap.seq.fetch here
                    bodies: [attachment.partID],
                    struct: true
                  });
                  f.on("message", (message,num)=>{
                    let buffer = '';
                    message.on("body", (stream, info) => {
